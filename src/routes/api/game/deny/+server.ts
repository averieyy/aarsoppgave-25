import { Client } from "$lib/client";
import { db } from "$lib/db";
import type { gamemember } from "$lib/types";
import { json } from "@sveltejs/kit";
import type { RequestHandler } from "./$types";

export const POST: RequestHandler = async ({ cookies, request }) => {
  const client = await Client.getClientFromCookies(cookies);
  if (!client) return json({ message: 'Unauthorized' }, { status: 403 });

  let body;

  try {
    body = await request.json();
  }
  catch {
    return json({ message: 'Body not JSON' }, { status: 400 });
  }

  const { speedrun }: { speedrun: number } = body;

  // Checking if the client is authorized
  const member = await db.queryOne<gamemember>('select m.* from speedrun s join games g on s.game_id = g.id join game_members m on m.game_id = g.id where s.id = $1::integer and m.admin = true and m.client_id = $2::integer', speedrun, client.id);
  if (!member) return json({ message: 'Unauthorized' }, { status: 403 });
  if (member.banned) return json({ message: 'You have been banned' }, { status: 403 });

  await db.execute('update speedrun set deleted = true where id = $1::integer', speedrun);

  return json({ message: 'Denied speedrun' }, { status: 200 });
}