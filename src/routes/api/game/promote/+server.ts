...
export const POST: RequestHandler = async ({ cookies, request }) => {
  ...
  if (!targetExists) return json({ message: 'This user can\'t be promoted' }, { status: 400 });
  if (targetExists.admin) return json({ message: 'This user is already an administrator' }, { status: 400 });

  await db.execute('update game_members set admin = true where client_id = $1::integer and game_id = $2::integer', targetExists.client_id, gameExists.id);

  return json({ message: 'Promoted to administrator' }, { status: 200 });
}