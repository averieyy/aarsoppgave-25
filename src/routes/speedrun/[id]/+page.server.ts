...
export const load: PageServerLoad = async ({ parent, params }) => {
  ...
  const speedrun: frontend_speedrun & { proof: string, proofmime: string, url_id: string, game: string, game_desc: string } = {category_id: s.category_label, deleted: s.deleted, description: s.description, id: s.id, score: s.score, submitted: s.submitted, username: s.username, verified: s.verified, url_id: s.url_id, game: s.game, game_desc: s.game_desc, proof: s.proof, proofmime: s.proofmime};
